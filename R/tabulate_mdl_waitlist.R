...
#' @title
#' @param data_waitlist
tabulate_mdl_waitlist <- function(data_waitlist, labels) {

  data_ref <- recipe(x = data_waitlist, time + status ~ .) %>%
  ...
  mdl_chd_1_full <- update(mdl_ref_full, data = data_chd_1)
  mdl_chd_1_uni <- update(mdl_chd_1_full, .~prim_dx_rc)

  mdl_chd_2_full <- update(mdl_ref_full,   data = data_chd_2)
  mdl_chd_2_uni <- update(mdl_chd_2_full, . ~ prim_dx_rc)

  mdl_chd_3_full <- update(mdl_ref_full,   data = data_chd_3)
  ...
  control_footer <-
    names(labels)[labels %in% c("age_listing",
                                "rabotype",
                                "hxneuro",
                                "dialysis",
                                "slecmo",
  ...
  mdl_cmy_1_full <- update(mdl_ref_full, data = data_cmy_1)
